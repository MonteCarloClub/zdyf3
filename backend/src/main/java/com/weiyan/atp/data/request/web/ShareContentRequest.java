...
//    @Value("${atp.pattern.attr}")
    ...
    public Result<Object> policyIsValid() {
        ...
        Matcher matcher = Pattern.compile(this.AttrPattern).matcher(str);
...
//                System.out.println("同一属性不能多次出现：" + attr);
        ...
        if (attrSet.size() <= 1) {
//            System.out.println("策略表达式格式错误：至少要有两个属性(xx:yy格式)");
            return Result.internalError("策略表达式格式错误：至少要有两个属性(xx:yy格式)");
        }
